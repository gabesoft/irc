...
var IRC = exports.IRC = function(options){
  ...
  self.on('hook::ready', function () {
    //
    // TODO: Check for auto-connect option
    //
    //self.connectClient();
  });
};
...
IRC.prototype._initIRC = function (data, cb) {
  var self = this;
  self.ircClient.on('motd', function (motd) {
    self.ircClient.say('nickserv', 'identify ' + self['password']);
    cb(null, { channels: self.channels, ircNick: self.nick });
    if (self['idCheck']) {
...
IRC.prototype.join = function (channel, cb) {
  var self = this;
  self.ircClient.join(channel, function () {
    cb(null, { channel: channel });
...
IRC.prototype.connectClient = function (data, cb) {
